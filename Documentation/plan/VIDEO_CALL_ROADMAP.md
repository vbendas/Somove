...
**Note on MVP:** The MVP (defined in `PRD.md`) uses Daily.co (not LiveKit) and includes contactless gesture controls via MediaPipe **Hands** — allowing therapist and client to control zoom, mute, volume, and camera from a distance using hand gestures. This roadmap describes the full marketplace video system with LiveKit and auto-framing. The MVP gesture controls (MediaPipe Hands) and the future auto-framing (MoveNet Pose) are independent features — one does not replace or depend on the other.
