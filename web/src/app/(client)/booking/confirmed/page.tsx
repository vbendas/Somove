...
export default async function BookingConfirmedPage({
  ...
  searchParams: Promise<{ sessionId?: string; payment?: string }>;
}) {
  const sp = await searchParams;
  if (!sp.sessionId) redirect("/my-sessions");
  ...
  const therapistName = therapist?.name || "Professional";
  ...
  return (
    <PageContainer width="narrow" className="max-w-md py-16 text-center">
        ...
        <div className="space-y-3">
          ...
          {session.payment_status === "paid" && (
            <div className="border-t border-border pt-3">
              ...
              <p className="font-medium text-foreground">
                {formatCurrency(session.amount_paid_cents)} — Confirmed
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          >
            <Button type="submit" className="w-full" size="lg">
              ...
              Complete Payment — {formatCurrency(session.amount_paid_cents)}
            </Button>
          </form>
        </div>
      )}
      ...
      {(isPaid || isFree || isConfirmed) && (
        <div className="mb-8 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <a
              href={`/api/calendar/${sp.sessionId}`}
              className="flex-1"
            >
              <Button variant="outline" className="w-full" size="sm">
                Download .ics
